...
import { extractPages, pdfBytesToBlob, countPdfPages } from "@/lib/pdf"
...
export default function PdfPageExtractor() {
  ...
  async function getExtractedBlob() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Could not extract these pages.")
      return null
    } finally {
      setProcessing(false)
    }
      ...
      {file && totalPages !== null && (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <DownloadButton
              getBlob={getExtractedBlob}
              filename={`${file.name.replace(/\.pdf$/i, "")}-extracted.pdf`}
              label={processing ? "Extracting..." : "Extract and download"}
